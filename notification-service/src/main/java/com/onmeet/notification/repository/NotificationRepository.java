...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
        ...
        @Modifying
        @Query("DELETE FROM Notification n WHERE n.createdAt < :cutoffDate AND NOT EXISTS (SELECT 1 FROM NotificationRecipient nr WHERE nr.notification.id = n.id)")
        int deleteOrphanNotifications(@Param("cutoffDate") LocalDateTime cutoffDate);
}
