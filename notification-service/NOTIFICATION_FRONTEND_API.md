...
알림 서비스에서 발생할 수 있는 주요 에러 코드입니다. 공통 에러 응답 포맷을 따릅니다.

| 코드 | HTTP | 메시지 | 설명 |
|------|------|--------|------|
| `NOTI_001` | 400 | SSE 구독에 필요한 사용자 ID가 없습니다 | SSE 연결 시 사용자 식별 불가 |
| `NOTI_010` | 404 | 해당 알림을 찾을 수 없습니다 | 존재하지 않거나 권한이 없는 알림 조작 시 |
| `NOTI_013` | 400 | FCM 토큰 또는 디바이스 ID가 누락되었습니다 | FCM 등록 요청 시 필드 누락 |
